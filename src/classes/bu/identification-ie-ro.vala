...
public class IdentificationIeRo : Identification {
       public IdentificationIeRo () {
            generate_first_part();
        ...
        protected override void generate_first_digit(){
            ...
            for(int i = 0; i < identification.core.length; i++){
                if(i < 5){
                    sum_totaly +=  int.parse(identification.core[i].to_string()) * peso_one;
                    peso_one--;

                }else{
                    sum_totaly +=  int.parse(identification.core[i].to_string()) * peso_two;
                    peso_two--;
                }
            }
